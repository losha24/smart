/* Smart Money Pro - js/ui.js - v6.5.7 - Back to 6.5.0 Design & Full Content */

let deferredPrompt;
let currentTab = 'home'; 

// --- מערכת ניווט (ביטול ריענון בשוק) ---
window.openTab = function(t) {
    ...
    setTimeout(() => {
        ...
        c.style.opacity = "1";
        window.scrollTo(0,0);
        // מניעת ריענון כפול בשוק כדי שלא יקפוץ
        if(typeof updateUI === 'function' && t !== 'market') updateUI();
    }, 120);
};

// --- דף הבית (עיצוב 6.5.0 המלא) ---
    ...
    c.innerHTML = `
        <div class="card fade-in">
            ...
            <div class="card" style="background:rgba(245, 158, 11, 0.05); border:1px solid rgba(245, 158, 11, 0.3); text-align:center; padding:15px; margin-bottom:15px;">
                <button id="giftBtn" onclick="window.claimDailyGift()" class="action-btn" style="width:100%; background:var(--yellow); color:#000; font-weight:bold; border:none; padding:10px; border-radius:8px; cursor:pointer;">🎁 קבלת בונוס</button>
...
// --- שחזור תוכן מלא מגרסה 6.5.0: כישורים ---
window.drawSkills = function(c) {
    c.innerHTML = `<h3>🎓 כישורים ולימודים</h3><div id="skills-list" class="grid-1"></div>`;
    const list = document.getElementById('skills-list');
    const skills = [
        {id:'s1', n:'קורס ניהול זמן', p:1000, xp:200, d:'מייעל את העבודה היומית'},
        {id:'s2', n:'אוריינות פיננסית', p:3500, xp:600, d:'הבנה בסיסית בשוק ההון'},
        {id:'s3', n:'שיווק ומכירות', p:8000, xp:1500, d:'מעלה את סיכויי ההצלחה בעסקים'},
        {id:'s4', n:'פיתוח אפליקציות', p:15000, xp:3500, d:'יכולת לבנות כלים דיגיטליים'},
        {id:'s5', n:'ניהול השקעות מתקדם', p:45000, xp:12000, d:'גישה לנכסים מורכבים'}
    ];
    skills.forEach(s => {
        const owned = window.skills && window.skills.includes(s.id);
        list.innerHTML += `
            <div class="card" style="border-right: 4px solid ${owned?'#22c55e':'#3b82f6'}">
                <b>${s.n}</b><br><small>${s.d}</small><br>
                <small>מחיר: ${s.p.toLocaleString()}₪ | +${s.xp} XP</small><br>
...
// --- שחזור תוכן מלא מגרסה 6.5.0: רכבים ---
window.drawCars = function(c) {
    c.innerHTML = `<h3>🚗 סוכנות רכבים</h3><div id="cars-list" class="grid-1"></div>`;
    const list = document.getElementById('cars-list');
    const cars = [
        {id:'c1', n:'קורקינט חשמלי', p:2500, img:'🛴'},
        {id:'c2', n:'אופנוע 125 סמ"ק', p:15000, img:'🛵'},
        {id:'c3', n:'מכונית משומשת', p:45000, img:'🚗'},
        {id:'c4', n:'ג׳יפ משפחתי', p:160000, img:'🚙'},
        {id:'c5', n:'רכב יוקרה', p:550000, img:'🏎️'}
    ];
    cars.forEach(car => {
        const owned = window.cars && window.cars.includes(car.id);
        list.innerHTML += `
            <div class="card" style="text-align:center">
                <div style="font-size:40px">${car.img}</div>
...
// --- לוגיקה פונקציונלית ותיקון מתנה ---
