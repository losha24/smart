/* Smart Money Pro - js/activities.js - v6.3.0 - Live Market Update */
...
setInterval(() => {
    stockList.forEach(s => {
        const change = (Math.random() * 0.05) - 0.024; // תנודה של +-2.5%
        s.price *= (1 + change);
        if (s.price < 1) s.price = 1;
        s.trend = change;
    });
    // רענון אוטומטי אם השחקן בלשונית הבורסה
...
function drawBusiness(c) { c.innerHTML = "<h3>💼 הקמת עסק</h3><p>בקרוב - ניהול עובדים והכנסה פסיבית ענקית!</p>"; }
