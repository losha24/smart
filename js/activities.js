const skPool = [{n:"רישיון נהיגה", c:5000}, {n:"רישיון לנשק", c:8000}, {n:"תעודת חובש", c:12000}, {n:"תכנות JS", c:45000}, {n:"קורס טיס", c:150000}];
...
const taskPool = [
    ...
    {n: "להפקיד לבנק", goal: 10000, r: 4000, type: 'bank'},
    {n: "קניית נכס/מוצר", goal: 1, r: 10000, type: 'buy'}
];

function initTasks() {
    if (activeTasks.length === 0) {
        activeTasks = [
            {...taskPool[0], id: Math.random(), cur: 0},
            {...taskPool[1], id: Math.random(), cur: 0},
            {id: 'gold', n: "משימת זהב: להרוויח 100K", goal: 100000, cur: 0, r: 50000, type: 'earn', isGold: true}
        ];
    }
}
...
function triggerTask(type, amt) {
    activeTasks.forEach((t, i) => {
        ...
        if(t.cur >= t.goal) {
            money += t.r; alert(`משימה הושלמה! +${t.r}₪`);
...
function drawWork(c) {
    ...
    jobs.forEach(j => {
        ...
        c.querySelector(".grid-2").innerHTML += `<div class="card" style="opacity:${has?1:0.5}"><b>${j.n}</b><br>
...
function startWork(p, t) {
    ...
    let i = setInterval(() => {
        ...
        if(s >= t) {
            ...
            triggerTask('work', 1); triggerTask('earn', p);
