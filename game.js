...
const jobs = [
    ...
    {n:"קבלן", p:8000, t:25, s:"תעודת טכנאי"},
...
const bzPool = [
    {n:"דוכן קפה", c:15000, p:120}, {n:"קיוסק", c:35000, p:350}, {n:"מכבסה", c:85000, p:900}, {n:"בר", c:250000, p:2800}, {n:"מסעדה", c:650000, p:7200}
];

const mkPool = [
    {n:"אייפון 15", c:4500}, {n:"רישיון לנשק", c:5000}, {n:"שעון רולקס", c:45000}, {n:"רכב", c:80000}, {n:"מחשב גיימינג", c:12000}
];

const skPool = [
    {n:"רישיון נהיגה", c:5000}, {n:"תעודת טכנאי", c:13000}, {n:"תכנות", c:65000}, {n:"ניהול", c:38000}
];
...
function openTab(tab) {
    ...
    if (tab === 'home') {
        ...
        c.innerHTML = `<div class="card"><h3>🏦 בנק</h3><p>יתרה: ${bank.toLocaleString()}₪</p>
            <input type="number" id="bankAmt" placeholder="סכום..." style="width:90%; padding:10px; margin-bottom:10px;">
            ...
            <hr><p>חוב: ${loan.toLocaleString()}₪</p><button class="action" onclick="bankOp('loan')" style="background:var(--main)">הלוואה 5000₪</button></div>`;
    }
    else if (tab === 'invest') {
        ...
        mkPool.forEach(m => c.querySelector(".grid-2").innerHTML += `<div class="card"><b>${m.n}</b><br>${m.c}₪<button class="action" onclick="buyItem('${m.n}',${m.c})">קנה</button></div>`);
    }
    else if (tab === 'skills') {
        c.innerHTML = `<div class="grid-2"></div>`;
        skPool.forEach(s => c.querySelector(".grid-2").innerHTML += `<div class="card"><b>${s.n}</b><br>${s.c}₪<button class="action" onclick="learn('${s.n}',${s.c})" ${skills.includes(s.n)?'disabled':''}>למד</button></div>`);
    }
}
...
function bankOp(type) {
    ...
    if(type === 'dep' && money >= val) { money -= val; bank += val; showMsg("הופקד", "pos"); }
    else if(type === 'wit' && bank >= val) { bank -= val; money += val; showMsg("נמשך", "pos"); }
    else if(type === 'loan') { loan += 5000; money += 5000; showMsg("הלוואה התקבלה", "neg"); }
    updateUI(); openTab('bank');
}

function buyStock(id, p) { if(money >= p*4) { money -= p*4; invOwned[id]++; showMsg("נקנה", "pos"); updateUI(); openTab('invest'); } }
function sellStock(id, p) { if(invOwned[id] > 0) { invOwned[id]--; money += p*4; showMsg("נמכר", "pos"); updateUI(); openTab('invest'); } }
function buyItem(n, c) { if(money >= c) { money -= c; inventory.push(n); passive += 10; showMsg("תתחדש!", "pos"); updateUI(); openTab('market'); } else showMsg("חסר כסף", "neg"); }
function learn(n, c) { if(money >= c && !skills.includes(n)) { money -= c; skills.push(n); showMsg("למדת כישור חדש", "pos"); updateUI(); openTab('skills'); } }
function claimGift() { const g = 5000+(level*500); if(Date.now()-lastGift>=14400000){ money+=g; lastGift=Date.now(); showMsg("+"+g, "pos"); updateUI(); openTab('home'); } }
